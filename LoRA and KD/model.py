...
class CausalSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        self.c_attn_aux= LoRALinear(config.n_embd, 3 * config.n_embd, config.LoRA_rank)
        # 2. add the auxiliary LoRA linear layer here corresponding to c_proj
        self.c_proj_aux= LoRALinear(config.n_embd, config.n_embd, config.LoRA_rank)
...
class MLP(nn.Module):
    def __init__(self, config):
        ...
        self.c_fc_aux= LoRALinear(config.n_embd, 4 * config.n_embd, config.LoRA_rank)
        self.c_proj_aux= LoRALinear(4 * config.n_embd, config.n_embd, config.LoRA_rank)
    ...
    def forward(self, x):
        ...
        x = self.c_fc(x) + self.c_fc_aux(x)
        x = self.gelu(x)
        x = self.c_proj(x) + self.c_proj_aux(x)
        x = self.dropout(x)
        return x
...
class LoRALinear(nn.Module):
    ...
    def forward(self, x):
        return x.matmul(self.U).matmul(self.V)  # x @ U @ V
